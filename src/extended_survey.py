...
from categories import cats, defs
...
def process_people_df(file_path):
    ...
    personas_cat["CLAVIVP"] = personas.CLAVIVP.map(defs["clavivp"]).astype(
        cats["clavivp"]
    )
    ...
    personas_cat["PARENTESCO"] = personas.PARENTESCO.map(defs["parentesco"]).astype(
        cats["parentesco"]
    )

    personas_cat["IDENT_MADRE"] = personas.IDENT_MADRE.replace(
        {96: "Otra vivienda", 97: "Falleció", 98: np.nan, 99: np.nan}
    )
    ...
    personas_cat["ENT_PAIS_NAC"] = personas.ENT_PAIS_NAC.apply(code_ent).astype(
        cats["ent"]
    )

    personas_cat["NACIONALIDAD"] = personas.NACIONALIDAD.map(defs["sino_139"]).astype(
        cats["sino_139"]
    )

    personas_cat["SERSALUD"] = personas.SERSALUD.map(defs["sersalud"]).astype(
        cats["sersalud"]
    )

    personas_cat["AFRODES"] = personas.AFRODES.map(defs["sino_139"]).astype(
        cats["sino_139"]
    )

    personas_cat["REGIS_NAC"] = personas.REGIS_NAC.map(
        {1: "México", 2: "Otro país", 3: "No tiene"}
    ).astype("category")
    ...
    personas_cat["RELIGION"] = personas.RELIGION.apply(code_religion).astype("category")

    personas_cat["DIS_VER"] = personas.DIS_VER.map(defs["dis"]).astype("category")
    personas_cat["DIS_OIR"] = personas.DIS_OIR.map(defs["dis"]).astype("category")
    personas_cat["DIS_CAMINAR"] = personas.DIS_CAMINAR.map(defs["dis"]).astype(
        "category"
    )
    personas_cat["DIS_RECORDAR"] = personas.DIS_RECORDAR.map(defs["dis"]).astype(
        "category"
    )
    personas_cat["DIS_BANARSE"] = personas.DIS_BANARSE.map(defs["dis"]).astype(
        "category"
    )
    personas_cat["DIS_HABLAR"] = personas.DIS_HABLAR.map(defs["dis"]).astype("category")
    personas_cat["DIS_MENTAL"] = personas.DIS_MENTAL.map(defs["dis_mental"]).astype(
        "category"
    )
    personas_cat["DIS"] = (
    ...
    personas_cat["HLENGUA"] = (
        personas["HLENGUA"].fillna(BPP).map({1: SI, 3: NO, BPP: BPP}).astype("category")
    )
    personas_cat["HESPANOL"] = (
        personas["HESPANOL"]
        .fillna(BPP)
        .map({1: SI, 3: NO, BPP: BPP})
        .astype("category")
    )

    personas_cat["ELENGUA"] = (
        personas.ELENGUA.fillna(BPP).map({5: SI, 7: NO, BPP: BPP}).astype("category")
    )

    personas_cat["PERTE_INDIGENA"] = (
        personas.PERTE_INDIGENA.fillna(BPP)
        .map({1: SI, 3: NO, BPP: BPP})
        .astype("category")
    )

    personas_cat["ASISTEN"] = (
        personas.ASISTEN.fillna(BPP).map({1: SI, 3: NO, BPP: BPP}).astype("category")
    )

    personas_cat["ENT_PAIS_ASI"] = personas.ENT_PAIS_ASI.apply(code_ent).astype(
        cats["ent"]
    )
    personas_cat["MUN_ASI"] = (
        pd.concat([personas[["MUN_ASI"]], personas_cat[["ENT_PAIS_ASI"]]], axis=1)
        .apply(code_MUN_ASI, axis=1)
        .astype("category")
    )

    personas_cat["TIE_TRASLADO_ESCU"] = (
        personas.TIE_TRASLADO_ESCU.fillna(BPP)
        .map(defs["tie_traslado"])
        .astype("category")
    )

    med_traslado_esc1_dummies = pd.get_dummies(
        personas.MED_TRASLADO_ESC1.fillna(BPP).map(defs["med_traslado"]),
        prefix="MED_TRASLADO_ESC",
    )
    med_traslado_esc2_dummies = pd.get_dummies(
        personas.MED_TRASLADO_ESC2.fillna(BPP).map(defs["med_traslado"]),
        prefix="MED_TRASLADO_ESC",
    )
    med_traslado_esc3_dummies = pd.get_dummies(
        personas.MED_TRASLADO_ESC3.fillna(BPP).map(defs["med_traslado"]),
        prefix="MED_TRASLADO_ESC",
    )
    ...
    personas_cat["ESCOLARI"] = personas.ESCOLARI.fillna(BPP).astype(
        pd.CategoricalDtype([0, 1, 2, 3, 4, 5, 6, 7, 8, BPP])
    )

    personas_cat["NOMCAR_C"] = personas.NOMCAR_C.fillna(BPP).replace(9999, np.nan)

    personas_cat["ALFABET"] = (
        personas.ALFABET.fillna(BPP).map({1: SI, 3: NO, BPP: BPP}).astype("category")
    )

    personas_cat["ENT_PAIS_RES_5A"] = personas.ENT_PAIS_RES_5A.apply(code_ent).astype(
        cats["ent"]
    )

    personas_cat["MUN_RES_5A"] = (
        pd.concat([personas[["MUN_RES_5A"]], personas_cat[["ENT_PAIS_RES_5A"]]], axis=1)
        .apply(code_MUN_ASI, axis=1, mun_col="MUN_RES_5A", ent_col="ENT_PAIS_RES_5A")
        .astype("category")
    )

    personas_cat["SITUA_CONYUGAL"] = (
        personas.SITUA_CONYUGAL.fillna(BPP)
        .map(
            {
                1: "unión libre",
                2: "separada(o)",
    ...
    personas_cat["OCUPACION_C"] = (
        np.floor(personas.OCUPACION_C / 10)
        .fillna(BPP)
        .map(
            {
    ...
    personas_cat["AGUINALDO"] = (
        personas.AGUINALDO.fillna(BPP)
        .map(defs["prestaciones"])
        .astype(cats["prestaciones"])
    )

    personas_cat["VACACIONES"] = (
        personas.VACACIONES.fillna(BPP)
        .map(defs["prestaciones"])
        .astype(cats["prestaciones"])
    )

    personas_cat["SERVICIO_MEDICO"] = (
        personas.SERVICIO_MEDICO.fillna(BPP)
        .map(defs["prestaciones"])
        .astype(cats["prestaciones"])
    )

    personas_cat["UTILIDADES"] = (
        personas.UTILIDADES.fillna(BPP)
        .map(defs["prestaciones"])
        .astype(cats["prestaciones"])
    )

    personas_cat["INCAP_SUELDO"] = (
        personas.INCAP_SUELDO.fillna(BPP)
        .map(defs["prestaciones"])
        .astype(cats["prestaciones"])
    )

    personas_cat["SAR_AFORE"] = (
        personas.SAR_AFORE.fillna(BPP)
        .map(defs["prestaciones"])
        .astype(cats["prestaciones"])
    )

    personas_cat["CREDITO_VIVIENDA"] = (
        personas.CREDITO_VIVIENDA.fillna(BPP)
        .map(defs["prestaciones"])
        .astype(cats["prestaciones"])
    )

    personas_cat["INGTRMEN"] = personas.INGTRMEN.fillna(BPP).replace(999999, np.nan)

    personas_cat["HORTRA"] = personas.HORTRA.fillna(BPP).replace(999, np.nan)

    personas_cat["ACTIVIDADES_C"] = (
        np.floor(personas.ACTIVIDADES_C / 100)
        .fillna(BPP)
        .map(defs["actividades"])
        .astype("category")
    )

    personas_cat["ENT_PAIS_TRAB"] = personas.ENT_PAIS_TRAB.apply(code_ent).astype(
        "category"
    )

    personas_cat["MUN_TRAB"] = (
        pd.concat([personas[["MUN_TRAB"]], personas_cat[["ENT_PAIS_TRAB"]]], axis=1)
        .apply(code_MUN_ASI, mun_col="MUN_TRAB", ent_col="ENT_PAIS_TRAB", axis=1)
        .astype("category")
    )

    personas_cat["TIE_TRASLADO_TRAB"] = (
        personas.TIE_TRASLADO_TRAB.fillna(BPP)
        .map(defs["tie_traslado_trab"])
        .astype("category")
    )

    med_traslado_trab1_dummies = pd.get_dummies(
        personas.MED_TRASLADO_TRAB1.fillna("Blanco por pase").map(defs["med_traslado"]),
        prefix="MED_TRASLADO_TRAB",
    )
    med_traslado_trab2_dummies = pd.get_dummies(
        personas.MED_TRASLADO_TRAB2.fillna("Blanco por pase").map(defs["med_traslado"]),
        prefix="MED_TRASLADO_TRAB",
    )
    med_traslado_trab3_dummies = pd.get_dummies(
        personas.MED_TRASLADO_TRAB3.fillna("Blanco por pase").map(defs["med_traslado"]),
        prefix="MED_TRASLADO_TRAB",
    )
    personas_cat = pd.concat(
        [
...
def categorize_p(personas):
    ...
    personas_cat["EDUC"] = (
        (personas.NIVACAD.astype(str) + "_" + personas.ESCOLARI.astype(str))
        .replace(
            {"nan_nan": np.nan, "Blanco por pase_Blanco por pase": "Blanco por pase"}
        )
        .map(nivacad_posmap)
        .astype("category")
    )
...
def process_places_df(file_path):
    ...
    viviendas_cat["CLAVIVP"] = viviendas.CLAVIVP.map(defs["clavivp"]).astype("category")

    viviendas_cat["PAREDES"] = (
        viviendas.PAREDES.fillna("Blanco por pase")
        .map(defs["paredes"])
        .astype(cats["paredes"])
    )

    viviendas_cat["TECHOS"] = (
        viviendas.TECHOS.fillna("Blanco por pase")
        .map(defs["techos"])
        .astype(cats["techos"])
    )

    viviendas_cat["PISOS"] = (
        viviendas.PISOS.fillna(BPP)
        .map(
            {
                1: "Tierra",
                2: "Cemento o firme",
    ...
    viviendas_cat["COCINA"] = (
        viviendas.COCINA.fillna("Blanco por pase")
        .map(defs["cocina"])
        .astype(cats["cocina"])
    )

    viviendas_cat["CUADORM"] = (
        viviendas.CUADORM.fillna(BPP).replace(99, np.nan).astype("category")
    )

    viviendas_cat["TOTCUART"] = (
        viviendas.TOTCUART.fillna(BPP).replace(99, np.nan).astype("category")
    )

    viviendas_cat["LUG_COC"] = (
        viviendas.LUG_COC.fillna("Blanco por pase")
        .map(defs["lug_coc"])
        .astype(cats["lug_coc"])
    )

    viviendas_cat["COMBUSTIBLE"] = (
        viviendas.COMBUSTIBLE.fillna("Blanco por pase")
        .map(defs["combustible"])
        .astype(cats["combustible"])
    )

    viviendas_cat["ESTUFA"] = (
        viviendas.ESTUFA.fillna("Blanco por pase")
        .map(defs["estufa"])
        .astype(cats["estufa"])
    )

    viviendas_cat["ELECTRICIDAD"] = (
        viviendas.ELECTRICIDAD.fillna(BPP)
        .map({1: SI, 3: NO, BPP: BPP})
        .astype("category")
    )

    viviendas_cat["FOCOS"] = (
        viviendas.FOCOS.fillna("Blanco por pase")
        .map(defs["focos"])
        .astype(cats["focos"])
    )

    viviendas_cat["FOCOS_AHORRA"] = (
        viviendas.FOCOS_AHORRA.fillna("Blanco por pase")
        .map(defs["focos_ahorra"])
        .astype(cats["focos_ahorra"])
    )

    viviendas_cat["AGUA_ENTUBADA"] = (
        viviendas.AGUA_ENTUBADA.fillna(BPP)
        .map(
            {
    ...
    viviendas_cat["ABA_AGUA_ENTU"] = (
        viviendas.ABA_AGUA_ENTU.fillna(BPP)
        .map(
            {
    ...
    viviendas_cat["ABA_AGUA_NO_ENTU"] = (
        viviendas.ABA_AGUA_NO_ENTU.fillna("Blanco por pase")
        .map(defs["aba_agua_no_entu"])
        .astype(cats["aba_agua_no_entu"])
    )

    viviendas_cat["TINACO"] = (
        viviendas.TINACO.fillna(BPP).map(defs["tinaco"]).astype(cats["tinaco"])
    )

    viviendas_cat["CISTERNA"] = (
        viviendas.CISTERNA.fillna(BPP).map(defs["cisterna"]).astype(cats["cisterna"])
    )

    viviendas_cat["BOMBA_AGUA"] = (
        viviendas.BOMBA_AGUA.fillna("Blanco por pase")
        .map(defs["bomba_agua"])
        .astype(cats["bomba_agua"])
    )

    viviendas_cat["REGADERA"] = (
        viviendas.REGADERA.fillna("Blanco por pase")
        .map(defs["regadera"])
        .astype(cats["regadera"])
    )

    viviendas_cat["BOILER"] = (
        viviendas.BOILER.fillna("Blanco por pase")
        .map(defs["boiler"])
        .astype(cats["boiler"])
    )

    viviendas_cat["CALENTADOR_SOLAR"] = (
        viviendas.CALENTADOR_SOLAR.fillna("Blanco por pase")
        .map(defs["calentador_solar"])
        .astype(cats["calentador_solar"])
    )

    viviendas_cat["AIRE_ACON"] = (
        viviendas.AIRE_ACON.fillna("Blanco por pase")
        .map(defs["aire_acon"])
        .astype(cats["aire_acon"])
    )

    viviendas_cat["PANEL_SOLAR"] = (
        viviendas.PANEL_SOLAR.fillna("Blanco por pase")
        .map(defs["panel_solar"])
        .astype(cats["panel_solar"])
    )

    viviendas_cat["SERSAN"] = (
        viviendas.SERSAN.fillna(BPP).map(defs["sersan"]).astype("category")
    )

    viviendas_cat["CONAGUA"] = (
        viviendas.CONAGUA.fillna(BPP)
        .map(
            {
    ...
    viviendas_cat["USOEXC"] = (
        viviendas.USOEXC.fillna("Blanco por pase")
        .map(defs["usoexc"])
        .astype(cats["usoexc"])
    )

    viviendas_cat["DRENAJE"] = (
        viviendas.DRENAJE.fillna(BPP)
        .map(
            {
                1: "La red pública.",
    ...
    viviendas_cat["SEPARACION1"] = (
        viviendas.SEPARACION1.fillna("Blanco por pase")
        .map(defs["separacion1"])
        .astype(cats["separacion1"])
    )

    viviendas_cat["SEPARACION2"] = (
        viviendas.SEPARACION2.fillna("Blanco por pase")
        .map(defs["separacion2"])
        .astype(cats["separacion2"])
    )

    viviendas_cat["SEPARACION3"] = (
        viviendas.SEPARACION3.fillna("Blanco por pase")
        .map(defs["separacion3"])
        .astype(cats["separacion3"])
    )

    viviendas_cat["SEPARACION4"] = (
        viviendas.SEPARACION4.fillna("Blanco por pase")
        .map(defs["separacion4"])
        .astype(cats["separacion4"])
    )

    viviendas_cat["DESTINO_BAS"] = (
        viviendas.DESTINO_BAS.fillna("Blanco por pase")
        .map(defs["destino_bas"])
        .astype(cats["destino_bas"])
    )

    viviendas_cat["REFRIGERADOR"] = (
        viviendas.REFRIGERADOR.fillna(BPP)
        .map(defs["refrigerador"])
        .astype(cats["refrigerador"])
    )

    viviendas_cat["LAVADORA"] = (
        viviendas.LAVADORA.fillna(BPP).map(defs["lavadora"]).astype(cats["lavadora"])
    )

    viviendas_cat["HORNO"] = (
        viviendas.HORNO.fillna(BPP).map(defs["horno"]).astype(cats["horno"])
    )

    viviendas_cat["AUTOPROP"] = (
        viviendas.AUTOPROP.fillna(BPP).map(defs["autoprop"]).astype(cats["autoprop"])
    )

    viviendas_cat["MOTOCICLETA"] = (
        viviendas.MOTOCICLETA.fillna(BPP)
        .map(defs["motocicleta"])
        .astype(cats["motocicleta"])
    )

    viviendas_cat["BICICLETA"] = (
        viviendas.BICICLETA.fillna(BPP).map(defs["bicicleta"]).astype(cats["bicicleta"])
    )

    viviendas_cat["RADIO"] = (
        viviendas.RADIO.fillna(BPP).map(defs["radio"]).astype(cats["radio"])
    )

    viviendas_cat["TELEVISOR"] = (
        viviendas.TELEVISOR.fillna(BPP).map(defs["televisor"]).astype(cats["televisor"])
    )

    viviendas_cat["COMPUTADORA"] = (
        viviendas.COMPUTADORA.fillna(BPP)
        .map(defs["computadora"])
        .astype(cats["computadora"])
    )

    viviendas_cat["TELEFONO"] = (
        viviendas.TELEFONO.fillna(BPP).map(defs["telefono"]).astype(cats["telefono"])
    )

    viviendas_cat["CELULAR"] = (
        viviendas.CELULAR.fillna(BPP).map(defs["celular"]).astype(cats["celular"])
    )

    viviendas_cat["INTERNET"] = (
        viviendas.INTERNET.fillna(BPP).map(defs["internet"]).astype(cats["internet"])
    )

    viviendas_cat["SERV_TV_PAGA"] = (
        viviendas.SERV_TV_PAGA.fillna(BPP)
        .map(defs["serv_tv_paga"])
        .astype(cats["serv_tv_paga"])
    )

    viviendas_cat["SERV_PEL_PAGA"] = (
        viviendas.SERV_PEL_PAGA.fillna(BPP)
        .map(defs["serv_pel_paga"])
        .astype(cats["serv_pel_paga"])
    )

    viviendas_cat["CON_VJUEGOS"] = (
        viviendas.CON_VJUEGOS.fillna(BPP)
        .map(defs["con_vjuegos"])
        .astype(cats["con_vjuegos"])
    )

    viviendas_cat["TENENCIA"] = (
        viviendas.TENENCIA.fillna("Blanco por pase")
        .map(defs["tenencia"])
        .astype(cats["tenencia"])
    )

    viviendas_cat["ESCRITURAS"] = (
        viviendas.ESCRITURAS.fillna("Blanco por pase")
        .map(defs["escrituras"])
        .astype(cats["escrituras"])
    )

    viviendas_cat["FORMA_ADQUI"] = (
        viviendas.FORMA_ADQUI.fillna("Blanco por pase")
        .map(defs["forma_adqui"])
        .astype(cats["forma_adqui"])
    )

    viviendas_cat["FINANCIAMIENTO1"] = (
        viviendas.FINANCIAMIENTO1.fillna("Blanco por pase")
        .map(defs["financiamiento1"])
        .astype(cats["financiamiento1"])
    )

    viviendas_cat["FINANCIAMIENTO2"] = (
        viviendas.FINANCIAMIENTO2.fillna("Blanco por pase")
        .map(defs["financiamiento2"])
        .astype(cats["financiamiento2"])
    )

    viviendas_cat["FINANCIAMIENTO3"] = (
        viviendas.FINANCIAMIENTO3.fillna("Blanco por pase")
        .map(defs["financiamiento3"])
        .astype(cats["financiamiento3"])
    )

    viviendas_cat["DEUDA"] = (
        viviendas.DEUDA.fillna("Blanco por pase")
        .map(defs["deuda"])
        .astype(cats["deuda"])
    )
    ...
    viviendas_cat["MCONMIG"] = (
        viviendas.MCONMIG.fillna("Blanco por pase")
        .map(defs["mconmig"])
        .astype(cats["mconmig"])
    )

    viviendas_cat["MNUMPERS"] = viviendas.MNUMPERS.fillna("Blanco por pase").astype(
        "category"
    )

    viviendas_cat["INGR_PEROTROPAIS"] = (
        viviendas.INGR_PEROTROPAIS.fillna("Blanco por pase")
        .map(defs["ingr_perotropais"])
        .astype(cats["ingr_perotropais"])
    )

    viviendas_cat["INGR_PERDENTPAIS"] = (
        viviendas.INGR_PERDENTPAIS.fillna("Blanco por pase")
        .map(defs["ingr_perdentpais"])
        .astype(cats["ingr_perdentpais"])
    )

    viviendas_cat["INGR_AYUGOB"] = (
        viviendas.INGR_AYUGOB.fillna("Blanco por pase")
        .map(defs["ingr_ayugob"])
        .astype(cats["ingr_ayugob"])
    )

    viviendas_cat["INGR_JUBPEN"] = (
        viviendas.INGR_JUBPEN.fillna("Blanco por pase")
        .map(defs["ingr_jubpen"])
        .astype(cats["ingr_jubpen"])
    )

    viviendas_cat["ALIMENTACION"] = (
        viviendas.ALIMENTACION.fillna("Blanco por pase")
        .map(defs["alimentacion"])
        .astype(cats["alimentacion"])
    )

    viviendas_cat["ALIM_ADL1"] = (
        viviendas.ALIM_ADL1.fillna("Blanco por pase")
        .map(defs["alim_adl1"])
        .astype(cats["alim_adl1"])
    )

    viviendas_cat["ALIM_ADL2"] = (
        viviendas.ALIM_ADL2.fillna("Blanco por pase")
        .map(defs["alim_adl2"])
        .astype(cats["alim_adl2"])
    )

    viviendas_cat["ING_ALIM_ADL1"] = (
        viviendas.ING_ALIM_ADL1.fillna("Blanco por pase")
        .map(defs["ing_alim_adl1"])
        .astype(cats["ing_alim_adl1"])
    )

    viviendas_cat["ING_ALIM_ADL2"] = (
        viviendas.ING_ALIM_ADL2.fillna("Blanco por pase")
        .map(defs["ing_alim_adl2"])
        .astype(cats["ing_alim_adl2"])
    )

    viviendas_cat["ING_ALIM_ADL3"] = (
        viviendas.ING_ALIM_ADL3.fillna("Blanco por pase")
        .map(defs["ing_alim_adl3"])
        .astype(cats["ing_alim_adl3"])
    )

    viviendas_cat["TIPOHOG"] = (
        viviendas.TIPOHOG.fillna(BPP).map(defs["tipohog"]).astype(cats["tipohog"])
    )

    viviendas_cat["INGTRHOG"] = viviendas.INGTRHOG.fillna(BPP).replace(999999, np.nan)

    viviendas_cat["JEFE_SEXO"] = viviendas.JEFE_SEXO.map(defs["sexo"]).astype(
        cats["sexo"]
    )
